  ...
  return new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        reject(
          new Error(
            'Location permission is required to submit attendance. Please allow location access and try again.'
          )
        );
      },
      {
...
export default function AttendanceForm() {
  ...
  useEffect(() => {
    async function loadSession() {
      ...
      try {
        ...
        setSessionError('Training session could not be found.');
      } finally {
  ...
  async function handleSubmit(event) {
    ...
    if (sessionId && sessionError) {
      setStatus('This training session is invalid. Please ask the instructor for a new link.');
      return;
    }

    if (sessionId && !session) {
      setStatus('Training session is still loading. Please wait a moment and try again.');
      return;
    }
  ...
  return (
    <section className="card">
      ...
      {sessionError && (
        <p className="status error">{sessionError}</p>
      )}
      ...
      {session && (
        <div className="session-banner">
          ...
          <p>
            <strong>Date:</strong> {session.training_date}
          </p>
          <p>
            <strong>Company:</strong> {session.company_name || 'Not provided'}
          </p>
          <p>
            <strong>Trainer:</strong> {session.trainer_name}
          </p>
          <p>
            <strong>Location:</strong> {session.training_location || 'Not provided'}
          </p>
        </div>
      )}
      ...
      <p className="muted">
        Enter your name, email, sign, and submit. Your browser will ask for
        location permission.
      </p>
      ...
      <form onSubmit={handleSubmit} className="form">
        ...
        <p className="location-note">
          Location permission is required to submit attendance. Please choose
